...
% --- Aplicar Kernel ---
if ~isempty(kernel_seleccionado)
    switch kernel_seleccionado
            case {'Sobel_X', 'Sobel_Y'}            
                kernel = kernels.(kernel_seleccionado);
                imagen_tratada = imfilter(imagen_double, kernel, 'replicate');
            case 'Ajustar Saturación'
                imagen_tratada = aplicar_filtro_hsv(imagen_tratada, 2.5); 
            case 'Disminuir Saturación'
                imagen_tratada = aplicar_filtro_hsv(imagen_tratada, 0.3); 
            otherwise
                if isfield(kernels, kernel_seleccionado)
                    kernel = kernels.(kernel_seleccionado);
                    imagen_tratada = imfilter(imagen_double, kernel, 'replicate');
                    imagen_tratada = (imagen_tratada - min(imagen_tratada(:))) / (max(imagen_tratada(:)) - min(imagen_tratada(:)));
                end
    end
end
...
function imagen_tratada_hsv = aplicar_filtro_hsv(imagen_rgb, saturacion_factor)
    % Convertir de RGB a HSV
    imagen_hsv = rgb2hsv(imagen_rgb);

    % Ajustar la saturación
    saturation_channel = imagen_hsv(:,:,2);
    saturation_channel = saturation_channel * saturacion_factor;
    saturation_channel(saturation_channel > 1) = 1;
    saturation_channel(saturation_channel < 0) = 0;
    imagen_hsv(:,:,2) = saturation_channel;

    % Convertir de nuevo a RGB
    imagen_tratada_hsv = hsv2rgb(imagen_hsv);
end